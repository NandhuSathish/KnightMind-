...
import { classifyMoves } from '../shared/cct/classifier.js';
...
chrome.runtime.onMessage.addListener((message: unknown, sender: chrome.runtime.MessageSender, sendResponse: (r: unknown) => void) => {
    // We handle async responses — must return true to keep the channel open
    void handleMessage(message, sender, sendResponse);
    return true;
});

async function handleMessage(message: unknown, sender: chrome.runtime.MessageSender, sendResponse: (r: unknown) => void): Promise<void> {
    ...
    switch (contentMsg.type) {
        ...
        case 'LOAD_REPERTOIRE': {
            const result: PGNLoadResult = await repertoireEngine.load(contentMsg.pgn, contentMsg.playerColor, contentMsg.filename).catch((err: unknown) => ({
                success: false,
                metadata: null,
                errors: [String(err)],
            }));
        ...
        case 'POSITION_CHANGED': {
            ...
            const { fen, orientation } = contentMsg;
            ...
            // Store the board orientation so broadcastToTabs can use the correct player side
            // even if settings.playerSide hasn't been manually updated by the user.
            registry.updateDetectedSide(tabId, orientation);
 ...
 */
async function broadcastToTabs(fen: string, lines: readonly RawPvLine[], baseHint: CoachingHint, playerSide: 'white' | 'black'): Promise<void> {
    const posKey = fenPositionKey(fen);
    ...
    await Promise.allSettled(
        tabs.map((tab) => {
            ...
            // Use the board orientation detected from the content script as the
            // authoritative "which color is the user" signal. Falls back to the
            // global settings value if the tab hasn't reported an orientation yet
            // (e.g., very first position after SW restart before any position change).
            const effectiveSide: 'white' | 'black' = state.detectedSide ?? playerSide;

            // ── Per-tab: opponent blunder detection ──────────────────────────────
            const punishment = state.prevEval ? detectOpponentBlunder(state.prevEval.score, state.prevEval.sideToMove, lines, fen, effectiveSide) : null;
            ...
            const myMoveQuality = state.lastEval !== null ? assessUserMove(state.lastEval.score, state.lastEval.sideToMove, currScore, currSideToMove, effectiveSide) : null;

            // ── Per-tab: opponent deviation detection ─────────────────────────────
            // `state.lastEval` (before advancing) is from the immediately preceding
            // position. If that position was the opponent's turn (sideToMove !== effectiveSide)
            ...
            const oppDeviated =
                ...
                state.lastInBook &&
                state.lastEval !== null &&
                state.lastEval.sideToMove !== effectiveSide;
...
function buildBaseHint(lines: readonly RawPvLine[], fen: string, difficulty: DifficultyLevel, repertoireMode: 'book' | 'engine', playerSide: 'white' | 'black'): CoachingHint {
    const best = lines[0];
    ...
    return {
        ...
        cctMoves: classifyMoves(fen, lines),
    };
}
...
function deriveTheme(lines: readonly RawPvLine[]): string | null {
    ...
    if (score.tag === 'mate') {
        return score.moves === 1 ? 'Checkmate in 1!' : `Forced checkmate in ${score.moves}`;
    }
    if (score.tag === 'cp') {
