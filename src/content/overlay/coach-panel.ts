...
export class CoachPanel {
    ...
    private readonly _cctSection: HTMLDivElement;
    private readonly _cctToggleBtn: HTMLButtonElement;
    private readonly _cctContent: HTMLDivElement;
    private readonly _bookSection: HTMLDivElement;
    private readonly _repHeaderLabel: HTMLSpanElement;
    private readonly _repToggleBtn: HTMLButtonElement;
    private readonly _bookContent: HTMLDivElement;
    ...
    private _bookCollapsed = false;
    private _cctCollapsed = false;
    ...
    /** Callback fired when a tactic item is hovered; null = leave. */
    private _onTacticHover: ((moveUCI: string | null) => void) | null = null;
    ...
    constructor() {
        ...
        // Tactic hover delegation — set once, survives innerHTML rewrites
        this._categories.addEventListener('mouseover', (e) => {
            const el = (e.target as Element).closest?.('.km-tactic-item') as HTMLElement | null;
            if (!el) return;
            const uci = el.dataset['uci'];
            if (uci) this._onTacticHover?.(uci);
        });
        this._categories.addEventListener('mouseout', (e) => {
            const rel = (e as MouseEvent).relatedTarget as Element | null;
            if (rel?.closest?.('.km-tactic-item')) return;
            this._onTacticHover?.(null);
        });
        ...
        const repHeaderRow = document.createElement('div');
        repHeaderRow.className = 'km-rep-header-row';
        this._repHeaderLabel = document.createElement('span');
        this._repHeaderLabel.className = 'km-rep-header';
        this._repToggleBtn = document.createElement('button');
        this._repToggleBtn.className = 'km-rep-toggle';
        this._repToggleBtn.setAttribute('aria-label', 'Toggle book moves');
        this._repToggleBtn.setAttribute('aria-expanded', 'true');
        this._repToggleBtn.textContent = '▼';
        repHeaderRow.append(this._repHeaderLabel, this._repToggleBtn);
        this._bookContent = document.createElement('div');
        this._bookContent.className = 'km-rep-content';
        this._bookSection.append(repHeaderRow, this._bookContent);
        ...
        // CCT (Checks → Captures → Threats) forcing moves panel
        this._cctSection = document.createElement('div');
        this._cctSection.className = 'km-cct hidden';
        const cctHeaderRow = document.createElement('div');
        cctHeaderRow.className = 'km-cct-header-row';
        const cctTitle = document.createElement('span');
        cctTitle.className = 'km-cct-title';
        cctTitle.textContent = '⚡ Forcing Moves';
        this._cctToggleBtn = document.createElement('button');
        this._cctToggleBtn.className = 'km-cct-toggle';
        this._cctToggleBtn.setAttribute('aria-label', 'Toggle forcing moves panel');
        this._cctToggleBtn.setAttribute('aria-expanded', 'true');
        this._cctToggleBtn.textContent = '▼';
        cctHeaderRow.append(cctTitle, this._cctToggleBtn);
        this._cctContent = document.createElement('div');
        this._cctContent.className = 'km-cct-content';
        this._cctSection.append(cctHeaderRow, this._cctContent);
        ...
        body.append(
            evalbarWrap,
            this._evalText,
            this._moveQualitySection,
            this._punisherSection,
            this._cctSection,
            this._sourceBadge,
            this._bookSection,
            this._coachLabel,
        ...
        this._repToggleBtn.addEventListener('click', () => this._toggleBookCollapse());
        this._cctToggleBtn.addEventListener('click', () => this._toggleCCTCollapse());
    ...
    /** Register a callback for tactic item hover; called with UCI on enter, null on leave. */
    setTacticHoverCallback(cb: (moveUCI: string | null) => void): void {
        this._onTacticHover = cb;
    }
    ...
    private _render(hint: CoachingHint): void {
        ...
        this._evalText.className = 'km-eval ' + (isPositive ? 'positive' : isNegative ? 'negative' : 'neutral');
        ...
        if (mq) {
            ...
            const lossStr = mq.cpLoss === 0 ? '±0.00' : `-${(mq.cpLoss / 100).toFixed(2)}`;
            this._moveQualitySection.className = `km-move-quality ${gradeClass}`;
            this._moveQualitySection.innerHTML =
                `<span class="km-mq-symbol">${esc(mq.symbol)}</span>` + `<span class="km-mq-label">${esc(mq.label)}</span>` + `<span class="km-mq-loss">${esc(lossStr)}</span>`;
        ...
        if (p) {
            ...
            const evalStr = p.evaluation >= 0 ? `+${(p.evaluation / 100).toFixed(2)}` : `${(p.evaluation / 100).toFixed(2)}`;
        ...
        // ── CCT forcing moves ─────────────────────────────────────────────────────
        const cct = hint.cctMoves;
        if (cct && (cct.checks.length > 0 || cct.captures.length > 0 || cct.threats.length > 0)) {
            this._cctContent.innerHTML = buildCCTHTML(cct);
            this._cctContent.classList.toggle('km-cct-content--hidden', this._cctCollapsed);
            this._cctToggleBtn.textContent = this._cctCollapsed ? '▶' : '▼';
            this._cctSection.classList.remove('hidden');
        } else {
            this._cctContent.innerHTML = '';
            this._cctSection.classList.add('hidden');
        }
        ...
        if (this._bookEnabled && rep?.source === 'book') {
            ...
            const infoHtml = infoParts.length > 0 ? `<div class="km-rep-line-info">${infoParts.join(' · ')}</div>` : '';

            // ── Next move suggestion + key ideas from annotations ─────────────────
            const movesHtml = rep.suggestedMoves
                .map((m) => {
                    ...
                    const ideaHtml = m.annotation ? `<div class="km-rep-ideas"><span class="km-rep-ideas-label">Key idea</span>${esc(m.annotation)}</div>` : '';

                    return (
                        `<div class="km-rep-entry">` +
            ...
            this._repHeaderLabel.textContent = `Book move${rep.suggestedMoves.length > 1 ? 's' : ''}:`;
            this._bookContent.innerHTML = `${infoHtml}${movesHtml}`;
            this._bookContent.classList.toggle('km-rep-content--hidden', this._bookCollapsed);
            this._repToggleBtn.textContent = this._bookCollapsed ? '▶' : '▼';
        ...
        const hasTactics = c !== null && c.tactics.length > 0;
        const hasCoaching = c !== null && (hasTactics || c.risk || c.strategic || c.positional || c.blunder);
        if (hasCoaching) {
            this._coachLabel.classList.remove('hidden');
            const tacticHtml = hasTactics
                ? c!.tactics.map(t =>
                    `<div class="km-category km-tactical km-tactic-item" data-uci="${esc(t.moveUCI ?? '')}">` +
                    `<span class="km-cat-label">Tactic</span>${esc(t.text)}</div>`
                  ).join('')
                : '';
    ...
    private _toggleBookCollapse(): void {
        this._bookCollapsed = !this._bookCollapsed;
        this._bookContent.classList.toggle('km-rep-content--hidden', this._bookCollapsed);
        this._repToggleBtn.textContent = this._bookCollapsed ? '▶' : '▼';
        this._repToggleBtn.setAttribute('aria-expanded', String(!this._bookCollapsed));
    }

    private _toggleCCTCollapse(): void {
        this._cctCollapsed = !this._cctCollapsed;
        this._cctContent.classList.toggle('km-cct-content--hidden', this._cctCollapsed);
        this._cctToggleBtn.textContent = this._cctCollapsed ? '▶' : '▼';
        this._cctToggleBtn.setAttribute('aria-expanded', String(!this._cctCollapsed));
    }
    ...
    private _syncBookName(): void {
        const name = this._playerSide === 'white' ? this._whiteBookName : this._blackBookName;
        if (name) {
...
import type { CCTResult, CCTMove } from '../../shared/cct/types.js';

function buildCCTHTML(cct: CCTResult): string {
    const rows: string[] = [];

    if (cct.checks.length > 0) {
        rows.push(buildCCTRow('♚', 'Checks', cct.checks));
    }
    if (cct.captures.length > 0) {
        rows.push(buildCCTRow('✖', 'Captures', cct.captures));
    }
    if (cct.threats.length > 0) {
        rows.push(buildCCTRow('⚡', 'Threats', cct.threats));
    }

    return rows.join('');
}

function buildCCTRow(icon: string, label: string, moves: CCTMove[]): string {
    const chips = moves
        .map((m) => {
            const isMate = m.mateIn !== undefined;
            const cls = isMate ? 'km-cct-chip--mate' : `km-cct-chip--${m.qualityClass}`;
            const approxCls = m.evalSource === 'heuristic' ? ' km-cct-chip--approx' : '';
            const qualEsc = esc(m.qualityLabel);
            const sanEsc = esc(m.moveSAN);
            return (
                `<span class="km-cct-chip ${cls}${approxCls}" title="${m.evalSource === 'engine' ? 'Engine eval' : 'Estimated'}">` +
                `<span class="km-cct-san">${sanEsc}</span>` +
                `<span class="km-cct-quality">${qualEsc}</span>` +
                `</span>`
            );
        })
        .join('');

    return `<div class="km-cct-row">` + `<span class="km-cct-row-label">${icon} ${esc(label)}</span>` + `<div class="km-cct-moves">${chips}</div>` + `</div>`;
}
